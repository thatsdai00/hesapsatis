...
import { FaArrowLeft, FaShoppingBag, FaCheck, FaTimes, FaSpinner, FaDownload, FaSync } from 'react-icons/fa';
          ...
          <div className="dashboard-card">
            ...
            {!selectedOrder ? (
              ...
              <div>
                ...
                {/* Product Stock / Account Details */}
                {orderDetails.stocks && orderDetails.stocks.length > 0 && (
                  <div className="mb-6">
                    <h4 className="font-medium mb-3 pb-2 border-b border-gray-700/50">Hesap Detayları</h4>
                    <div className="space-y-4">
                      {orderDetails.stocks.map((stock: any) => (
                        <div 
                          key={stock.id} 
                          className="bg-black/20 p-4 rounded-lg border border-gray-800 relative"
                          onClick={handleAccountDetailsClick}
                        >
                          <div className="flex justify-between items-center mb-3 relative z-30">
                            <p className="font-medium">{stock.product.name}</p>
                            <button 
                              className="text-primary hover:text-primary-hover bg-black/30 p-2 rounded-full transition-colors z-50"
                              title="Hesap detaylarını indir"
                              onClick={(e) => handleDownload(e, stock.content, stock.product.name)}
                              type="button"
                              aria-label="Hesap detaylarını indir"
                            >
                              <FaDownload size={14} />
                            </button>
                          </div>
                          <div className="relative z-30">
                            <AccountCredential content={stock.content} />
                          </div>
                        </div>
                      ))}
                    </div>
