...
function fillMissingDates(
  dateMap: Map<string, { date: string; totalAmount: number; orderCount: number }>,
  startDate: Date,
  endDate: Date,
  timeRange: 'today' | 'week' | 'month' | 'year' | 'all'
): Map<string, { date: string; totalAmount: number; orderCount: number }> {
...
export const adminRouter = createTRPCRouter({
  ...
  getSalesData: adminProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const now = new Date();
      let startDate: Date;
      let ordersByDate: Array<{ date: string; totalAmount: number; orderCount: number }> = [];
